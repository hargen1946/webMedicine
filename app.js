...
function getQrHistory(){try{const v=JSON.parse(localStorage.getItem(QR_HISTORY_KEY)||'[]');return Array.isArray(v)?v:[]}catch{return[]}}
function rememberQrFingerprints(fingerprints){const history=new Set(getQrHistory());fingerprints.filter(Boolean).forEach(f=>history.add(f));localStorage.setItem(QR_HISTORY_KEY,JSON.stringify([...history].slice(-1000)))}
...
function renderHome(){const count=state.qrList.length;app.innerHTML=`<section class="hero"><h1>お 薬 手 帳</h1><p>処方箋QRを読み取り、保存します。</p></section>${count?`<div class="summary-card"><strong>✓ ${count}件</strong> のQRコードを読み取りました</div>`:''}<div class="button-stack home-actions"><button class="button secondary" data-action="history">▤ 記 録 を 見 る</button><button class="button ${count?'orange':'primary'}" data-action="scan">▦ ${count?'次のQRコードを読む':'QRコードを読み取る'}</button>${count?'<button class="button success wide" data-action="finish">読み取り終了・保存</button>':''}</div>${state.notice?`<div class="notice">${esc(state.notice)}</div>`:''}<div class="utility-row"><button class="button ghost" data-action="export">CSV出力</button><button class="button ghost" data-action="backup">バックアップ</button><label class="button ghost file-button">復元<input id="json-import" type="file" accept="application/json"></label></div><button class="help-link" data-action="help">使い方・データ保存について</button>`;app.querySelector('[data-action="history"]').onclick=()=>navigate('history');app.querySelector('[data-action="scan"]').onclick=openScanner;app.querySelector('[data-action="finish"]')?.addEventListener('click',finishReading);app.querySelector('[data-action="export"]').onclick=exportCsv;app.querySelector('[data-action="backup"]').onclick=()=>download(JSON.stringify(getRecords(),null,2),'お薬手帳バックアップ.json','application/json');app.querySelector('#json-import').onchange=importJson;app.querySelector('[data-action="help"]').onclick=()=>navigate('help')}
function renderHelp(){app.innerHTML=`<h1 class="page-title">使い方・データ保存について</h1><div class="help-sections"><section><h2>CSV出力とは</h2><p>お薬の記録を、パソコンなどで一覧表として見るためのファイルに保存します。</p><p class="help-note">CSVファイルから、このお薬手帳へ記録を戻すことはできません。</p></section><section><h2>バックアップとは</h2><p>すべてのお薬の記録を、機種変更や故障に備えてファイルに保存します。</p><p>保存された「お薬手帳バックアップ.json」は大切に保管してください。</p><p class="help-note">新しい記録を追加した後にバックアップしておくと安心です。少なくとも、機種変更やブラウザのデータを削除する前には必ず行ってください。</p></section><section><h2>復元とは</h2><p>以前に保存したバックアップファイルを選び、記録をこの端末のお薬手帳へ戻します。</p><p class="help-note">機種変更後、新しい端末へ記録を移すときに行います。普段の使用では必要ありません。</p><ol><li>古い端末で「バックアップ」を押します。</li><li>保存されたファイルを新しい端末へ移します。</li><li>新しい端末で「復元」を押し、そのファイルを選びます。</li></ol></section><section><h2>記録の保存場所</h2><p>記録はサーバーへ送信されず、この端末のブラウザ内だけに保存されます。</p><p class="help-note">機種変更やブラウザのデータ削除をする前に、必ずバックアップしてください。</p></section></div><div class="bottom-actions"><button class="button secondary" data-action="home">ホームへ戻る</button></div>`;app.querySelector('[data-action="home"]').onclick=()=>navigate('home')}
...
function display(v){return String(v).replaceAll('﨑','崎').replaceAll('髙','高').replaceAll('神','神').replaceAll('塚','塚').trim()}
...
let installPrompt=null;const installButton=document.querySelector('#install-button'),installGuide=document.querySelector('#install-guide-dialog'),installGuideNext=document.querySelector('#install-guide-next'),installGuideBack=document.querySelector('#install-guide-back');window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();installPrompt=e;installButton.classList.remove('hidden')});installButton.onclick=()=>{if(!installPrompt){installButton.classList.add('hidden');return}installGuide.showModal()};installGuideBack.onclick=()=>installGuide.close();installGuide.addEventListener('cancel',()=>installGuide.close());installGuideNext.onclick=async()=>{installGuide.close();if(!installPrompt)return;const prompt=installPrompt;installPrompt=null;installButton.classList.add('hidden');await prompt.prompt()};window.addEventListener('appinstalled',()=>{installPrompt=null;installButton.classList.add('hidden');if(installGuide.open)installGuide.close()});
