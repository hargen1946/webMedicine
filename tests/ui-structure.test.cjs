...
assert.match(app, /記録を安全に保つために/, '記録を安心して扱うための説明が必要です');
...
assert.match(app, /上の2個のボタンの説明/, 'ホームに2個の補助ボタンの説明リンクが必要です');
assert.match(app, /data-action="delete">この記録を削除する/, '説明書と同じ削除ボタン名が必要です');
assert.doesNotMatch(app, /data-action="help">使い方・データ保存について/, 'ホームの説明リンクは2個のボタンを明示します');
